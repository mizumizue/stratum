---
name: stratum-adopt
description: Adopt Stratum into projects — probe repository structure, create deterministic backups, and apply non-destructive overlay or full clean-root restructure.
---

# Adopt Stratum

Apply Stratum's V-Model traceability matrix and test stratum analyzer to any repository.
...
```bash
./bin/stratum adopt "<targetDir>" --mode restructure
```
...
Prove that the adopted project satisfies all Stratum rules:
1. Run schema and cycle validation in the target project:
   ```bash
   ./bin/stratum check -d "<targetDir>/docs"
   ```
...
4. If validation fails and cannot be resolved immediately, execute rollback:
   ```bash
   ./bin/stratum adopt "<targetDir>" --rollback "<backupDir>"
   ```

**Completion criterion**: `./bin/stratum check` completes with exit code 0.
...
- **Rollback one-liner**:
  ```bash
  ./bin/stratum adopt "<targetDir>" --rollback "<targetDir>/.stratum-backup/<timestamp>_<mode>"
  ```
- **CLI Options**:
