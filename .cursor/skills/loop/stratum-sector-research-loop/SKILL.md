---
name: stratum-sector-research-loop
...
テーマ語の連想や「小規模店舗」一般論で止めず、**sector brief** を一次に近い根拠まで掘る。仕様・PBI・実装には進まない。後工程（仕様深度・デザイン・Stratum Spike 等）の入力になる。
...
| 起動 | 人間との接点 |
|------|----------------|
| 上位オーケストレータの1工程（例: `stratum-validation-loop-workflow`） | **なし**。テーマ・`STATE_DIR`・打ち切りは呼び出し側の指示書に従う。判断はオーケストレータに任せる |
