...
3. **プロダクト設計指針への完全不干渉 (ADR-0008)**: `src/` 配下の製品コードは特定のアーキテクチャ（MVC、レイヤード、DDD、ヘキサゴナル等）やプログラミング言語に限定されず、プロダクト自体の内部設計指針には一切関わらない。
4. 仕様・決め事の正本は `docs/`（NEED, REQ, SPEC, DSN, ADR, QA, TC）の V-Model 体系とする（仕様は What、製品コードは How）。
