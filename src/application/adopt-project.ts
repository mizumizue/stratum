...
function generateBinWrappers(): Record<string, string> {
  const stratumBash = `#!/usr/bin/env bash
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"

# Try local stratum package or fallback to global / sibling
if [[ -f "$ROOT/src/node_modules/.bin/stratum" ]]; then
...
echo {"ok":false,"error":"Stratum CLI not found. Please install stratum globally or add to dependencies."} 1>&2
exit /b 1
`;

  const stratumPs1 = `$ErrorActionPreference = "Stop"
...
Write-Error '{"ok":false,"error":"Stratum CLI not found. Please install stratum globally or add to dependencies."}'
exit 1
`;
...
export function adoptProject(options: AdoptionOptions = {}): {
  ...
  if (!silent) {
    console.log(`\n\x1b[32m✔ Stratum successfully applied in [${mode}] mode!\x1b[0m`);
    console.log(`  - Files created: ${createdFiles.length}`);
    console.log(`  - To verify docs schema:  ./bin/stratum check (or npx stratum check)`);
    if (backupDir) {
      console.log(`  - To rollback if needed:   ./bin/stratum adopt --rollback "${backupDir}"`);
    }
    console.log('');
  }
