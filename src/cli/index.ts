...
program
  .name('stratum')
  .description('Stratum - V-Model Traceability Matrix & Test Stratum Sufficiency Analyzer')
  .version('0.1.0');
...
program
  ...
  .action((options) => {
    ...
    if (options.format === 'json') {
      ...
      const lines: string[] = [
        '# Stratum - Test Case Input Modifiability Analysis\n',
...
program
  ...
  .action((options) => {
    ...
    if (options.format === 'json') {
      ...
      const lines: string[] = [
        '# Stratum - Decisions & Architecture Catalog\n',
...
program
  ...
  .action(async (options) => {
    ...
    server.listen(port, () => {
      console.log(`\n\x1b[32m🚀 Stratum Dashboard is running at:\x1b[0m \x1b[1mhttp://localhost:${port}/\x1b[0m`);
...
program
  .command('mcp')
  .description('Start Stratum MCP server for Cursor / AI Agent integration')
  .option('-d, --docs <dir>', 'Docs directory path')
  .action(async (options) => {
    const docsDir = resolveDocsDir(options.docs);
...
program
  ...
  .description('Adopt Stratum into an existing or new project (supports overlay and restructure modes with backup)')
...
<head>
  ...
  <title>Stratum - Quality & Traceability Dashboard</title>
    ...
    <header class="flex items-center justify-between pb-6 border-b border-slate-800">
      <div>
        <div class="flex items-center gap-3">
          ...
          <h1 class="text-3xl font-extrabold bg-gradient-to-r from-teal-400 to-blue-500 bg-clip-text text-transparent">Stratum</h1>
        </div>
