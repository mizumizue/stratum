...
- **特定アーキテクチャ・言語に限定しない (ADR-0008)**: `src/` 配下の製品コードの設計指針（アーキテクチャ・パターン・モジュール構成）には一切関わらない。プロダクトの完全な裁量。
- **決め事（What）> コード**: 仕様や設計契約が正本（What vs How の厳格分離）。
- **実用性とコード配置の共存**: Stratum の可視化のために製品コード（言語・構成・設計）の自由度や保守性を損なわない。特定言語やフレームワークを前提とせず、あらゆる技術スタックの製品開発を支援する。
