 ...
 * - 期待結果: ルートに node_modules や dist が存在せず、bin/stratum ラッパー経由で依存関係が透過的に解決される構成であること
...
test('TC-0030: クリーンルート規約およびbinラッパー構成の外部結合検証', () => {
  ...
  const binStratumSh = path.join(rootDir, 'bin/stratum');
  const binStratumCmd = path.join(rootDir, 'bin/stratum.cmd');
  const binStratumPs1 = path.join(rootDir, 'bin/stratum.ps1');

  assert.ok(fs.existsSync(binStratumSh), 'bin/stratum must exist');
  assert.ok(fs.existsSync(binStratumCmd), 'bin/stratum.cmd must exist');
  assert.ok(fs.existsSync(binStratumPs1), 'bin/stratum.ps1 must exist');

  assert.ok(fs.statSync(binStratumSh).isFile());
  assert.ok(fs.statSync(binStratumCmd).isFile());
  assert.ok(fs.statSync(binStratumPs1).isFile());
});
