...
本書は、**Stratum**（ストラータム / 旧称: TraceWeave）の機能追加・修正・リファクタリングを行う開発者（人間のエンジニアおよび AI エージェント）向けの実装ガイドラインです。

当プロジェクトでは、**lucid** の設計指針を尊重し、実装コードを直接書き始める「コードファースト」を避け、指示の抽象度に応じた文書（要求または要件）を起点として精緻化・検証を経てから実装に着手する**文書駆動開発（Doc-First / Spec-Driven）**、ならびに**リポジトリトップレベル（Root）のクリーン維持**を徹底しています。
