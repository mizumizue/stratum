...
また、**ForgOS の工程手順（`agents/`、各モード）**に則り、作業時は指示の抽象度に応じて `Spike` / `Specify` / `Implement` / `Audit` / `Steward` を意識して進めます。特定の可視化画面（Web ダッシュボード等）に開発全体が引っ張られることはありません。製品内部のアーキテクチャ設計やコード構造（How）はプロダクト側の自律的な判断に委ねられます。
