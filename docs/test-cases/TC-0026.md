...
### Preconditions
`buildStratumReport` モジュールおよび実ドキュメント群（`docs/`）が参照可能であること。

### Steps
1. リポジトリの実ドキュメントディレクトリを明示して `buildStratumReport` を実行し、生成された `StratumDataPayload` の構造（summary, strata, pyramid, matrix, gaps, catalog）を検証する。
