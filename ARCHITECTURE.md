...
言語やフレームワーク固有の実装資産・依存関係（例: Node.js の `package.json` や `node_modules/`、Python の `pyproject.toml` や仮想環境、Rust の `Cargo.toml` や `target/` 等）を含むすべての製品実装資産は `src/` 配下に完全カプセル化する。ルート直下に個別の開発資材、ビルド成果物、依存キャッシュディレクトリを一切露出させない。すべての CLI コマンドは `bin/stratum`（または `bin/traceweave`）ラッパーが透過実行する。
